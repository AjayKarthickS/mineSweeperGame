...
public class SweeperBoard {
	...
	private void placeMines(int userInputX, int userInputY){
		...
		for(int i = 0; i<numberOfMines;){
			...
			while(mineflag[random] != true && x != userInputX && y != userInputY){
	...
	public int playMineSweeperUtil(int userInputX, int userInputY){
		...
		if(movesLeft == (board.length*board.length) - numberOfMines){ // checks if this is user's first move
			placeMines(userInputX,userInputY);
			computeShadowBoard();
		}
	...
	public void printShadowBoard(){
		...
		for(int i = 0 ; i < shadowBoard.length; i++){
			System.out.print(i+"   ");
			for(int j = 0; j < shadowBoard.length; j++){
				System.out.print(shadowBoard[i][j]+" ");
			}
			System.out.println();
		}
